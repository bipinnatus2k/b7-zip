...
use gpui::{block_on, App, AppContext, Application, AsyncApp, PromptButton, QuitMode, Task, TaskExt, UpdateGlobal as _};
use gpui_platform;
                ...
                window
                    .update(cx, |_, window, cx| {
                        let response = window.prompt(
                            ...
                            "message",
                            Some(&"error_details"),
                            &["Exit","Jump Once","Always Jump","Overwrite","Overwrite Always"],
                            cx,
                        );
...
struct Args {
    ...
    #[arg(short, long, action = clap::ArgAction::Append, num_args = 2, value_names = ["OLD_PATH", "NEW_PATH"])]
    diff: Vec<String>,
...
fn check_for_conpty_dll() {
    ...
    if let Ok(hmodule) = unsafe { LoadLibraryW(w!("conpty.dll")) } {
        unsafe {
            FreeLibrary(hmodule)
                .context("Failed to free conpty.dll").expect("TODO: panic message")
                ;
        }
    } else {
