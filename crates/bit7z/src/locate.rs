...
pub fn locate_dll() -> Option<PathBuf> {
    ...
    let vcpkg_bin = std::env::var("VCPKG_ROOT")
        .ok()
        .map(|root| PathBuf::from(root).join("installed/x64-windows/bin"));
...
mod tests {
    ...
    fn exe_dir_prefers_7z_dll_over_7zip_dll() {
        ...
        assert_eq!(locate_dll_in(dir.path(), None), Some(dir.path().join("7z.dll")));
    }
    ...
    fn exe_dir_falls_back_to_7zip_dll() {
        let dir = tempfile::tempdir().unwrap();
        touch(&dir.path().join("7zip.dll"));
        assert_eq!(locate_dll_in(dir.path(), None), Some(dir.path().join("7zip.dll")));
    }
