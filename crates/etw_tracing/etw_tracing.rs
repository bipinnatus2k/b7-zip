        ...
        unsafe {
            if let Ok(object_type) = info.GetObjectType() {
                let name = match object_type {
                    ObjectType_Profile => "Profile",
                    ObjectType_Collector => "Collector",
                    ObjectType_Provider => "Provider",
                    _ => "Unknown",
                };
                let _ = write!(out, "\n  Object type: {name}");
            }
...
fn create_wpr<T: Interface>(clsid: &windows_core::GUID) -> Result<T> {
    unsafe {
...
pub struct EtwSession {
    ...
    pub stream: BufReader<net::UnixStream>,
...
pub fn send_json<T: serde::Serialize>(writer: &mut impl Write, value: &T) -> Result<()> {
...
pub fn recv_json<T: serde::de::DeserializeOwned>(reader: &mut impl BufRead) -> Result<T> {
...
pub struct EtwSessionHandle {
    pub writer: net::OwnedWriteHalf,
    pub _listener: net::UnixListener,
    pub socket_path: PathBuf,
}
