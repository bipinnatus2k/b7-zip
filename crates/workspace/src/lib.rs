...
//     }
//
//     /// Entry point used by the thin app binary.
//     ///
//     /// Positional file paths that look like archives are opened; otherwise
//     /// the complete path list is preloaded into the Add-to-archive dialog.
//     /// This keeps the app CLI free of `--add`/debug/config switches.
//     pub fn open_paths(&mut self, paths: &[PathBuf], cx: &mut Context<Self>) {
//         let Some(first) = paths.first() else { return };
//         if paths
//             .iter()
//             .any(|path| !bit7z_explorer::is_archive_name(&path.to_string_lossy()))
//         {
//             self.open_add_dialog(paths.to_vec(), cx);
//         } else {
//             self.open_archive(first, cx);
//         }
//     }
//
...
//                     cx.subscribe(&explorer, |this, explorer, event: &ExplorerEvent, cx| {
...
//                             "Archives",
//                             &[
//                                 "7z", "zip", "tar", "gz", "tgz", "bz2", "xz", "wim", "rar", "cab",
//                                 "iso", "lzma", "zst",
//                             ],
//                         )
...
//         let Some(archive) = self.current_archive.clone() else {
//             return;
//         };
...
//                 .background_executor()
//                 .spawn(async move {
//                     let dest_root = temp::temp_root().join("open");
...
//                         },
//                     )?;
//                     Ok(dest_root.join(temp::sanitize_relative(Path::new(&entry_path))))
//                 })
//                 .await;
//             match result {
//                 Ok(out) => {
...
//             Some(0) => OverwriteSpec::Overwrite,
//             Some(1) => OverwriteSpec::Skip,
//             Some(2) => OverwriteSpec::Ask,
//             _ => OverwriteSpec::AutoRename,
//         };
...
//     }
//
//     pub fn open_add_dialog(&mut self, files: Vec<PathBuf>, cx: &mut App) {
...
//             if inputs
//                 .iter()
//                 .any(|input| input == &target || (input.is_dir() && target.starts_with(input)))
//             {
//                 self.toast_err(
//                     "Output archive is inside an input path; choose another name".into(),
//                     cx,
//                 );
//                 return;
//             }
...
//             return;
//         };
//         let Some(row) = explorer.read(cx).focused_row(cx) else {
//             return;
//         };
...
//             self.toast_err(
//                 "Names cannot contain path separators or parent-directory segments".into(),
//                 cx,
//             );
//             return;
//         }
...
//                 )
//                 .child(
//                     Text::new("Open an archive to start")
//                         .size(Size::Md)
//                         .dimmed(),
//                 )
//                 .child(
//                     Button::new("empty-open", "Open archive…")
...
//     {
//         // Use ShellExecuteW instead of `cmd /C start`: the latter interprets
//         // `&`, `|`, `^` and friends in archive-derived file names as shell
//         // metacharacters.
//         let path = windows::core::HSTRING::from(path);
//         let _ = unsafe {
