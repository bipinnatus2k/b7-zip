...
fn main() {
    println!("cargo:rerun-if-changed=../crates/resources/executor.rc");
    ...
    let Some(rc) = find_rc() else {
        println!("cargo:warning=rc.exe not found; skipping Windows resources for bit7z-executor");
        return;
    };

    let resources = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../crates/resources");
    let out_dir = PathBuf::from(std::env::var("OUT_DIR").unwrap());
    let res_out = out_dir.join("executor.rc".replace(".rc", ".res"));

    let status = Command::new(&rc)
        .current_dir(&resources)
        .arg("/fo")
        .arg(&res_out)
        .arg(&resources.join("executor.rc"))
        .status()
        .expect("failed to spawn rc.exe");
    if !status.success() {
        panic!("rc.exe failed for executor.rc");
    }
